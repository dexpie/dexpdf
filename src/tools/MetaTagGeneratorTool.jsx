            ...
            <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-card p-6 rounded-3xl shadow-lg border border-border space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-muted-foreground uppercase">Robots</label>
                            <select
                                value={data.robots}
                                onChange={e => setData({ ...data, robots: e.target.value })}
                                className="w-full p-3 bg-secondary rounded-xl font-bold text-sm outline-none"
                            >
                        ...
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-muted-foreground uppercase">Theme Color</label>
                            <div className="flex gap-2">
                                ...
                                <input type="text" value={data.themeColor} onChange={e => setData({ ...data, themeColor: e.target.value })} className="flex-1 bg-secondary rounded-xl px-2 text-sm font-bold" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-slate-900 rounded-3xl shadow-xl overflow-hidden flex flex-col h-[500px]">
                    <div className="bg-slate-800 p-4 border-b border-slate-700 flex justify-between items-center">
                        ...
                        <button onClick={() => navigator.clipboard.writeText(generateCode())} className="text-muted-foreground hover:text-white flex items-center gap-2">
        ...
        <div className="space-y-1">
            ...
            {textarea ? (
                <textarea
                    ...
                    className="w-full p-3 bg-secondary border border-border rounded-xl outline-none focus:ring-2 ring-purple-500 font-bold text-foreground text-sm h-24 resize-none"
                />
            ) : (
                <input
                    value={value} onChange={e => onChange(e.target.value)}
                    className="w-full p-3 bg-secondary border border-border rounded-xl outline-none focus:ring-2 ring-purple-500 font-bold text-foreground text-sm"
                />
            )}
        </div>
    )
