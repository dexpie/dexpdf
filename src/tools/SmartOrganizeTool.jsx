...
export default function SmartOrganizeTool() {
    ...
    return (
        <ToolLayout title="Smart Organizer" description="Suggest names and folders using first-page text patterns.">
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="flex justify-between items-center bg-card p-4 rounded-xl border border-border shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-bold text-foreground">Document Analysis</h3>
                                <p className="text-xs text-muted-foreground">{isProcessing ? 'Analyzing content...' : `${analyzedFiles.length} files processed`}</p>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            <button
                                onClick={() => setFiles([])}
                                className="px-4 py-2 text-muted-foreground hover:bg-secondary rounded-lg text-sm font-medium transition-colors"
                            >
                                Reset
                            </button>
                            {isDone && (
                    ...
                    <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
                        <div className="grid grid-cols-12 gap-4 p-4 bg-secondary border-b border-border text-xs font-bold text-muted-foreground uppercase tracking-wider">
                        ...
                        <div className="divide-y divide-slate-100">
                            {analyzedFiles.map((item) => (
                                <motion.div
                                    ...
                                    className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-secondary transition-colors"
                                >
                                    {/* Original */}
                                    <div className="col-span-4 flex items-center gap-3 overflow-hidden">
                                        <FileText className="w-8 h-8 text-muted-foreground flex-shrink-0" />
                                        <div className="min-w-0">
                                            <p className="text-sm font-medium text-foreground truncate" title={item.originalName}>{item.originalName}</p>
