            ...
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="bg-slate-900 text-white p-8 rounded-3xl text-center shadow-lg relative overflow-hidden">
                    <div className="relative z-10">
                        <div className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-2">Current Unix Timestamp</div>
                        <div className="text-6xl font-black font-mono tracking-tighter text-green-400 mb-4">{now}</div>
                        <div className="flex gap-4 justify-center">
                            <button onClick={() => setNow(prev => prev)} className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm font-bold">Refresh</button>
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100">
                        ...
                        {output && (
                            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                                <div className="text-xs font-bold text-slate-400 uppercase mb-1">Result</div>
                                <div className="font-bold text-slate-800 break-words">{output}</div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100">
                        ...
                        {reverseOutput && (
                            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 flex justify-between items-center">
                                <div>
                                    <div className="text-xs font-bold text-slate-400 uppercase mb-1">Epoch</div>
                                    <div className="font-bold text-slate-800 font-mono">{reverseOutput}</div>
                                </div>
                                <button onClick={() => navigator.clipboard.writeText(reverseOutput)} className="text-slate-400 hover:text-blue-500">
