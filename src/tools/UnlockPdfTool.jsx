...
import { Unlock, Lock, File as FileIcon, Eye, EyeOff, ShieldCheck, AlertCircle, KeyRound } from 'lucide-react'
...
export default function UnlockPdfTool() {
    ...
    async function handleFiles(files) {
        ...
        // Try to detect if encrypted by just loading it? 
        // pdf-lib's load throws if encrypted. If not encrypted, warn user.
        try {
            const array = await f.arrayBuffer()
            await PDFDocument.load(array)
            // If successful, it's NOT encrypted
            setErrorMsg('This PDF is not password protected.')
        } catch (err) {
            if (err.message.includes('encrypted')) {
                // Good, it is encrypted
            }
        }
    }
    ...
    async function unlockPdf() {
        if (!file) return
        if (password.length < 1) { setErrorMsg('Please enter the password'); return }
        ...
        try {
            ...
            // PDFDocument.load accepts a password option
            const pdf = await PDFDocument.load(array, { password })
            ...
            a.href = url
            a.download = getOutputFilename(outputFileName, file.name.replace(/\.pdf$/i, '') + '_unlocked')
            a.click()
            URL.revokeObjectURL(url)
            setSuccessMsg(t('unlock.success', 'PDF unlocked successfully!'))
            triggerConfetti()
        } catch (err) {
            console.error(err)
            if (err.message.includes('Incorrect password')) {
                ...
                setErrorMsg(t('common.error_process', 'Failed to unlock PDF: ' + err.message))
            }
        } finally {
            setBusy(false)
        }
    ...
    return (
        <ToolLayout title="Unlock PDF" description={t('tool.unlock_desc', 'Remove password security from your PDF')}>
                ...
                {!file ? (
                    ...
                    >
                        ...
                        <div className="space-y-6 mb-8">
                            <div className="relative">
                                <label className="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                                    <KeyRound className="w-4 h-4 text-slate-400" /> Enter Password
                                </label>
                                <div className="relative">
                                    <input
                        ...
                        <div className="bg-slate-50 p-4 rounded-xl mb-6">
                            ...
                            <FilenameInput value={outputFileName} onChange={e => setOutputFileName(e.target.value)} placeholder="unlocked" />
                        </div>

                        <div className="flex flex-col gap-3">
                            <ActionButtons
                                primaryText="Unlock PDF"
                                onPrimary={unlockPdf}
                                loading={busy}
                                icon={Unlock}
                            />
