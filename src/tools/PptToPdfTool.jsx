...
export default function PptToPdfTool() {
  ...
  async function convert() {
    if (!file) { setErrorMsg('Please select a PPTX file first.'); return; }
    setErrorMsg(''); setSuccessMsg('');
    setBusy(true)
    try {
      ...
      if (images.length > 0) {
        ...
        setSuccessMsg('Success! PDF created and downloaded.');
      } else {
        const out = new JSZip()
        for (let i = 0; i < images.length; i++) {
        ...
        setSuccessMsg('Success! Slide images ZIP downloaded.');
      }
  ...
  async function processBatchFile(f, index, onProgress) {
    try {
      ...
      if (images.length === 0) {
        throw new Error('No slide images found in PPTX. Only supports PPTX with slide images.')
      }
      ...
      throw new Error(`Failed to convert PPTX to PDF: ${err.message || err}`)
    }
  }
