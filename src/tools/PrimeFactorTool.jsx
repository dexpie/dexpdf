    ...
    return (
        <ToolLayout title="Prime Factorization" description="Break numbers down into primes.">
            <div className="max-w-2xl mx-auto space-y-8">
                <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-slate-100">
                    <div className="flex gap-4">
                        <input
                            ...
                            className="flex-1 text-2xl font-black p-4 bg-slate-50 rounded-xl outline-none focus:ring-2 ring-indigo-500"
                        />
                ...
                {result && (
                    <div className={`p-8 rounded-[2rem] shadow-lg border-2 text-center transition-all ${result.isPrime ? 'bg-green-50 border-green-200' : 'bg-white border-slate-100'}`}>
                        <div className="mb-4">
                            {result.isPrime ? (
                        ...
                        <div className="text-slate-500 font-bold mb-2">Prime Factors</div>
                        <div className="flex flex-wrap justify-center gap-2">
                            {result.factors.map((f, i) => (
                                <React.Fragment key={i}>
                                    <span className="text-4xl font-black text-slate-800">{f}</span>
                                    {i < result.factors.length - 1 && <span className="text-2xl text-slate-300 font-bold py-2">×</span>}
                                </React.Fragment>
                            ))}
                        </div>
                    </div>
