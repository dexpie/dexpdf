            ...
            <div className="max-w-6xl mx-auto flex flex-col gap-8 h-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-[400px]">
                    <div className="flex flex-col gap-2">
                        <label className="font-bold text-slate-500 text-sm">Original Text</label>
                        <textarea
                            value={oldText}
                            onChange={e => setOldText(e.target.value)}
                            className="flex-1 p-4 rounded-2xl border border-slate-200 outline-none focus:border-red-400 focus:ring-4 focus:ring-red-100 transition-all font-mono text-xs resize-none"
                            placeholder="Paste original text here..."
                        />
                    </div>
                    <div className="flex flex-col gap-2">
                        <label className="font-bold text-slate-500 text-sm">New Text</label>
                        <textarea
                            value={newText}
                            onChange={e => setNewText(e.target.value)}
                            className="flex-1 p-4 rounded-2xl border border-slate-200 outline-none focus:border-green-400 focus:ring-4 focus:ring-green-100 transition-all font-mono text-xs resize-none"
                ...
                <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100 flex flex-col">
                    ...
                    <div className="p-0 overflow-auto max-h-[500px] bg-slate-900 text-slate-300 font-mono text-sm leading-relaxed">
                        {differences.length === 0 ? (
                            <div className="p-8 text-center text-slate-500 italic">Differences will appear here...</div>
                        ) : (
                            differences.map((part, index) => {
                                const color = part.added ? 'bg-green-900/30 text-green-400 border-l-4 border-green-500' :
                                    part.removed ? 'bg-red-900/30 text-red-400 border-l-4 border-red-500' :
                                        'text-slate-400 border-l-4 border-transparent hover:bg-slate-800'
