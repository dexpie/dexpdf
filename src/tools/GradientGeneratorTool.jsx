            ...
            <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-8">
                ...
                <div className="w-full md:w-80 space-y-6">
                    <div className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100 space-y-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Color 1</label>
                                <div className="flex gap-2">
                                    <input type="color" value={color1} onChange={e => setColor1(e.target.value)} className="w-10 h-10 rounded-lg cursor-pointer" />
                                    <input type="text" value={color1} onChange={e => setColor1(e.target.value)} className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-3 font-mono text-sm uppercase" />
                                </div>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Color 2</label>
                                <div className="flex gap-2">
                                    <input type="color" value={color2} onChange={e => setColor2(e.target.value)} className="w-10 h-10 rounded-lg cursor-pointer" />
                                    <input type="text" value={color2} onChange={e => setColor2(e.target.value)} className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-3 font-mono text-sm uppercase" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex gap-2 p-1 bg-slate-100 rounded-xl">
                                <button onClick={() => setType('linear')} className={`flex-1 py-1 rounded-lg text-xs font-bold ${type === 'linear' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400'}`}>Linear</button>
                                <button onClick={() => setType('radial')} className={`flex-1 py-1 rounded-lg text-xs font-bold ${type === 'radial' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400'}`}>Radial</button>
                            </div>
                        </div>
                ...
                <div className="flex-1 space-y-6">
                    ...
                    <div className="bg-slate-900 text-green-400 p-6 rounded-2xl font-mono text-sm shadow-lg overflow-x-auto flex justify-between items-center group">
                        ...
                        <button onClick={copy} className="p-2 hover:bg-white/10 rounded-lg text-white">
