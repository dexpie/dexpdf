        ...
        <ToolLayout title="URL Parser" description="Break down URLs into protocol, host, path, and queries.">
            <div className="max-w-4xl mx-auto flex flex-col gap-8">
                ...
                <div className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100">
                    <label className="text-sm font-bold text-slate-500 uppercase mb-2 block">Enter URL</label>
                    <input
                        type="text"
                        value={url}
                        onChange={e => setUrl(e.target.value)}
                        className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-700 outline-none focus:ring-2 ring-blue-500"
                ...
                {parsed ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100 space-y-4">
                            <h3 className="font-bold text-xl text-slate-800 mb-4 border-b pb-2">Components</h3>

                            <div className="flex items-center gap-4">
                                <div className="p-3 bg-purple-100 text-purple-600 rounded-lg"><Globe className="w-5 h-5" /></div>
                                <div>
                                    <div className="text-xs font-bold text-slate-400 uppercase">Protocol</div>
                                    <div className="font-mono font-bold text-slate-700">{parsed.protocol}</div>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="p-3 bg-blue-100 text-blue-600 rounded-lg"><Database className="w-5 h-5" /></div>
                                <div>
                                    <div className="text-xs font-bold text-slate-400 uppercase">Host</div>
                                    <div className="font-mono font-bold text-slate-700">{parsed.hostname} {parsed.port ? `:${parsed.port}` : ''}</div>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="p-3 bg-orange-100 text-orange-600 rounded-lg"><Folder className="w-5 h-5" /></div>
                                <div>
                                    <div className="text-xs font-bold text-slate-400 uppercase">Path</div>
                                    <div className="font-mono font-bold text-slate-700">{parsed.pathname}</div>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="p-3 bg-green-100 text-green-600 rounded-lg"><Link className="w-5 h-5" /></div>
                                <div>
                                    <div className="text-xs font-bold text-slate-400 uppercase">Hash</div>
                                    <div className="font-mono font-bold text-slate-700">{parsed.hash || '-'}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Query Params */}
                        <div className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100">
                            <h3 className="font-bold text-xl text-slate-800 mb-4 border-b pb-2 flex items-center gap-2">
                            ...
                            {Object.keys(parsed.params).length > 0 ? (
                                ...
                                <div className="text-slate-400 text-center py-10 italic">No query parameters found.</div>
                            )}
                        </div>
                    </div>
                ) : (
